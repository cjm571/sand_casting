...
use crate::{game_assets::colors, profiler};
...
impl WeatherManager {
    ...
    pub fn update_weather(&mut self, ci_ctx: &CastIronContext, ggez_ctx: &mut GgEzContext) {
        ...
        if elapsed_time.as_millis() >= self.timeout_ms {
            ...
            self.profiler
                .mark_event(String::from("WEATHER_GEN_START"), ggez_ctx)
                .unwrap();
            ...
            // Send WEATHER_GEN event marker to profiler
            self.profiler
                .mark_event(String::from("WEATHER_GEN_STOP"), ggez_ctx)
                .unwrap();
        }
        ...
        if self.prev_intensity != cur_intensity || new_weather_generated {
            // Send WEATHER_GEN event marker to profiler
            self.profiler
                .mark_event(String::from("WEATHER_CHANGE_START"), ggez_ctx)
                .unwrap();
            ...
            self.hud_elements
                .update_content_mesh(content_color, ggez_ctx);

            // Update intensity text
            self.hud_elements
                .update_text_elements(self.active_weather.element(), cur_intensity);
            ...
            // Send WEATHER_GEN event marker to profiler
            self.profiler
                .mark_event(String::from("WEATHER_CHANGE_STOP"), ggez_ctx)
                .unwrap();
        }

        // Update intensity bar
        self.hud_elements.update_int_bar_mesh(
            self.active_weather
                .intensity_exact(elapsed_time.as_secs_f64()),
            ci_ctx,
            ggez_ctx,
        );
    }
...
impl HudElements {
    ...
    fn default(ci_ctx: &CastIronContext, ggez_ctx: &mut GgEzContext) -> Self {
        ...
        let mut hud_elements = Self {
            ...
            int_bar_mesh: ggez_gfx::MeshBuilder::new()
                ...
                .build(ggez_ctx)
                .unwrap(),
            text_elem_pos: ggez_mint::Point2 {
                x: calc_frame_pos.x,
    ...
    pub fn draw(&self, ggez_ctx: &mut GgEzContext) {
        ...
        ggez_gfx::draw(
            ggez_ctx,
            &self.text_int_obj,
            (self.text_int_pos, 0.0, colors::GREEN),
        )
        .unwrap();
        ggez_gfx::draw(
            ggez_ctx,
            &self.text_elem_obj,
            (self.text_elem_pos, 0.0, colors::GREEN),
        )
        .unwrap();
    ...
    fn update_frame_mesh(&mut self, ggez_ctx: &mut GgEzContext) {
        ...
        let outline_rect = ggez_gfx::Rect::new(
            self.frame_pos.x,
            self.frame_pos.y,
            self.frame_size,
            self.frame_size,
        ...
        self.frame_mesh = ggez_gfx::Mesh::new_rectangle(
            ggez_ctx,
            ggez_gfx::DrawMode::stroke(HUD_OUTLINE_LINE_WIDTH),
            outline_rect,
            HUD_OUTLINE_LINE_COLOR,
        )
        .unwrap();
    }
    ...
    fn update_content_mesh(&mut self, color: ggez_gfx::Color, ggez_ctx: &mut GgEzContext) {
        // Build a square in the top-right of the screen to hold the weather info
        let color_rect = ggez_gfx::Rect::new(
            self.frame_pos.x,
            self.frame_pos.y,
            self.frame_size,
            self.frame_size,
        ...
        self.content_mesh =
            ggez_gfx::Mesh::new_rectangle(ggez_ctx, ggez_gfx::DrawMode::fill(), color_rect, color)
                .unwrap();
    }

    /// Updates the mesh for the HUD intensity bar
    fn update_int_bar_mesh(
        &mut self,
        exact_intensity: f64,
        ci_ctx: &CastIronContext,
        ggez_ctx: &mut GgEzContext,
        ...
        let dummy_line = [
            ggez_mint::Point2 { x: 0.0, y: 0.0 },
            ggez_mint::Point2 { x: 1.0, y: 1.0 },
        ];
        int_bar_mesh_builder
            .line(&dummy_line, 1.0, colors::TRANSPARENT)
            .unwrap();

        let drawable_intensity: f32 =
            (exact_intensity as f32 / ci_ctx.max_weather_intensity() as f32) * self.frame_size;

        // Build a square in the top-right of the screen to hold the weather info
        let int_bar_line = [
            ggez_mint::Point2 {
                x: self.frame_pos.x - 5.0,
        ...
        self.int_bar_mesh = int_bar_mesh_builder
            .line(&int_bar_line, HUD_INT_BAR_LINE_WIDTH, colors::GREEN)
            .unwrap()
            .build(ggez_ctx)
            .unwrap();
    }
    ...
    fn update_text_elements(&mut self, element: Element, intensity: weather::Intensity) {
        ...
        self.text_elem_obj = ggez_gfx::Text::new((
            self.text_elem_str.as_str(),
        ...
        self.text_int_obj = ggez_gfx::Text::new((
            self.text_int_str.as_str(),
            ggez_gfx::Font::default(),
            crate::DEFAULT_TEXT_SIZE,
        ));
    }
